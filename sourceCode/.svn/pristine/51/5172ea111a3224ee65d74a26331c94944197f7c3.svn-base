...
public class StaticIpSet {
	...
	public void confingStaticIp(WifiConfiguration wifiConfig) {
		...
		if (android.os.Build.VERSION.SDK_INT < 11) {
			...
			Settings.System
					.putInt(ctRes, Settings.System.WIFI_USE_STATIC_IP,
							1);
		...
		// 如果是android3.x版本及以上的话
			...
			try {
				...
				Log.i("123", "静态ip设置成功！");
				Toast.makeText(mContext, "静态ip设置成功！",
						Toast.LENGTH_SHORT).show();
			} catch (Exception e) {
				e.printStackTrace();
				Log.i("123", "静态ip设置失败！");
				Toast.makeText(mContext, "静态ip设置失败！",
						Toast.LENGTH_SHORT).show();
			}
		}
	}
	...
	private void setGateway(InetAddress gateWay, WifiConfiguration wifiConfig)
		...
		// android3.x版本
			...
			ArrayList<InetAddress> gateWays = (ArrayList<InetAddress>)
					getDeclaredField(
	...
	private void setIpType(String ipType, WifiConfiguration wifiConfig)
			throws Exception {
		Field[] fs = wifiConfig.getClass().getFields();
