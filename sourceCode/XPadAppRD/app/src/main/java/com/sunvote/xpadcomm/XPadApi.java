...
//			mInputStream = mSerialPort.getInputStream();
			...
			client.enableModuleLog();
			client.enablelogtoFile();
//			client.disableLogtoFile();
//			client.disableModuleLog();
...
//						Log.d(TAG, "send vote time:" + sendTimes + " serialNo:" + serialNo);
						...
						if (sendTimes >= 6) {
							...
							m_listener.onVoteSubmitError(VoteResultItem.this);
							break;
						}
						try {
							Thread.sleep(2000);
	...
	private VoteResultItem removeSentItem(int serialNo) {
		...
		synchronized (sendQueen) {
			for (int i = 0; i < sendQueen.size(); i++) { // 删除完成的
				VoteResultItem it = (VoteResultItem) sendQueen.get(i);
				if (it.status == 1 && it.serialNo == (byte)serialNo) {
					it.sendOk = true;
					ret = it;
					sendQueen.remove(it);
	...
	@Override
	public synchronized void submitVote(int ansType, String info) {// byte[] ansData
...
//			int iVal = ChoiceValueToInt(val);
	...
	public void submitVoteAllOK() {
		...
		VoteResultItem it = new VoteResultItem();
		it.retBack = "" + AnsType_BatchSingle;
		it.status = 0;
		it.ansType = AnsType_BatchSingle;
		it.serialNo = (byte)serialNumber;
		allOkSerialNumber = (byte)serialNumber;
		it.ansCount = 1;
		it.allOK = 1;
		it.data = mBuffer;
		addToSendQueen(it);
	...
	private class ReadThread extends Thread {
		...
		public void run() {
			...
			while (!isInterrupted()) {
				...
				try {
					...
					// byte[] buffer = new byte[512];// buffer 64字节
					if (mInputStream == null)
						return;
					size = mInputStream.read(comBuffer);

					if (size > 0 && m_listener != null) {
						// m_listener.onComData(buffer,size);
						// if(isShowLog){
						// printDataBuf(buffer,size,"comRecv:");
						// }
						int recvLen = size;
		...
		private void checkComData(byte[] data, int size) {
			...
			// if(!Crc16.checkPack(data)){
			// Log.d(TAG, "wrong packge");
			// }else{
			// int crcValue =Crc16.getUnsignedShort( Crc16.crc16(data,size -4-2)
			// );
			// int originCrcValue = 0;
			// originCrcValue |= (data[size-2] &0xff) << 8 ;
			// originCrcValue |= data[size-1] & 0xff;
			//
			// if(crcValue != originCrcValue){
			// System.out.println("recv package crc error!\n");
			// return;
		...
		private void responseVoteSendSuccess(byte[] data, int size) {
			...
			VoteResultItem item = removeSentItem(serialNo);

			if ((byte)serialNo == allOkSerialNumber) {
				m_listener.onVoteSubmitAllOkSuccess();
			} else {
				m_listener.onVoteSubmitSuccess(item);
			}
		}
