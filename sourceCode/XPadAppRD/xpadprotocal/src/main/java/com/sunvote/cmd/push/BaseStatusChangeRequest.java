 ...
 * <p>
 * 字节	标识符	描述
 * 1	REQUEST_CMD	0x71 基础信标变化
 * 2	BADDH	基站编号
 * 3	IDMODE	基站识别模式，1编号、2序列号、
 * SDK要依据此模式来判断提交数据时候的打包格式
 * 4-5	CONFID	2字节，会议资料UID编号，1-65535，高位字节在前
 * （对应系统使用后创见的会议文件的唯一编号）
 * 6	VOTEID	议题编号或索引（1-255），对应到具体用户待机时候可以浏览的议案文件内容
 * 值=0，表示不浏览内容 ，值=255，表示自由浏览所有内容
 * 其他值，指定议案编号浏览
 * 7-8	AUTHCODE	授权号，2字节，高位在前，0-0xFFFF
 * =0时候不使用授权模式，表决器可以参与表决
 * >0时候，表决器保存的授权号相同才能参与表决
 * 9	LOGIN	登录申请模式（后台签到模式），是否需要IC卡、登录码（用户编号、
 * 10	REPORT  * + * LANGUAGE	表决器报告状态模式和指定语言
 * 11	OFFTIME	自动关机时间
 * 12	ATTRIB	表决器特性：背光模式+蜂鸣器模式等等
 * 13-24	BASENAME	基站名称，最多12字节
 */

public class BaseStatusChangeRequest extends PushBaseCmd {

    public static final byte REQUEST_CMD = 0x71;

    public static  final  int CMD_LENGTH  = 24 ;

    public BaseStatusChangeRequest(){}

    public BaseStatusChangeRequest(BaseStatusChangeResponse response){
